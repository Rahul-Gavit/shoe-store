      ...
      <div className="flex justify-between items-start gap-20 flex-wrap max-lg:flex-col ">
        ...
        <div className="flex flex-1 justify-between lg:gap-10 gap-20 flex-wrap">
          {footerLinks.map((foot) => (
            <div key={foot}>
              <h3 className="text-white font-montserrat leading-normal mb-6 text-3xl ">
                {foot.title}
              </h3>
              <ul>
                {foot.links.map((link) => (
                  <li
                    key={link.name}
                    className=" text-white font-montserrat hover:text-slate-gray cursor-pointer mt-3 text-base leading-normal"
                  >
